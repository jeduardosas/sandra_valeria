export const dataBoda = {
  nombre_novia:'',
  nombre_novio:'',
  nombre_papa_novia:'',
  nombre_mama_novia:'',
  nombre_papa_novio:'',
  nombre_mama_novio:'',
  nombre_padrino:'',
  nombre_madrina:'',
  frase:'',
  img_header:'',
  img_fondo1:'',
  img_fondo2:'',
  fecha_dia:'',
  fecha_mes:'',
  fecha_year:'',
  religiosa_hora:'',
  religiosa_minutos:'',
  religiosa_lugar:'',
  religiosa_direccion:'',
  religiosa_direccion_col:'',
  religiosa_direccion_cd:'',
  religiosa_ubicacion:'',
  recepcion_hora:'',
  recepcion_minutos:'',
  recepcion_lugar:'',
  recepcion_direccion:'',
  recepcion_direccion_col:'',
  recepcion_direccion_cd:'',
  recepcion_ubicacion:'',
  confirmacion_novia:'',
  confirmacion_novio:'',
  regalos_frase:'',
  regalos_url:'',
  time_line:{
    settings:{
      bgColor:'',
      color:'',
      lineColor:''
    },
    protocol:[
      {
        id:1,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },

      {
        id:2,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },

      {
        id:3,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
      {
        id:4,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
      {
        id:5,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
    ]
  },
  hoteles:[
    {
      nombre:'',
      direccion:'',
      direccion_colonia:'',
      telefono:'',
      ubicacion:''
    },
    {
      nombre:'',
      direccion:'',
      direccion_colonia:'',
      telefono:'',
      ubicacion:''
    },
    {
      nombre:'',
      direccion:'',
      direccion_colonia:'',
      ubicacion:''
    }
  ]
}

export const dataBautizo={
  nombre:'',
  nombre_papa:'',
  nombre_mama:'',
  nombre_padrino:'',
  nombre_madrina:'',
  frase:'',
  img_header:'',
  img_banner1:'',
  img_banner2:'',
  fecha_dia:'',
  fecha_mes:'',
  fecha_year:'',
  religiosa_hora:'',
  religiosa_minutos:'',
  religiosa_lugar:'',
  religiosa_direccion:'',
  religiosa_direccion_col:'',
  religiosa_direccion_cd:'',
  religiosa_ubicacion:'',
  recepcion_hora:'',
  recepcion_minutos:'',
  recepcion_lugar:'',
  recepcion_direccion:'',
  recepcion_direccion_col:'',
  recepcion_direccion_cd:'',
  recepcion_ubicacion:'',
  confirmacion:'',
  regalos_frase:'',
  regalos_url:'',
  time_line:{
    settings:{
      bgColor:'',
      color:'',
      lineColor:''
    },
    protocol:[
      {
        id:1,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },

      {
        id:2,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },

      {
        id:3,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
      {
        id:4,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
      {
        id:5,
        time:'',
        name:'',
        icon:'',
        color:'',
        textColor:''
      },
    ]
  },
  hoteles:[
    
  ]
}

export const dataXV={
  nombre:'Sandra Valeria',
  nombre_papa:'Silviano Rojas Trejo',
  nombre_mama:'Sandra María Chacón Herrera',
  nombre_padrino:'Hector Hugo Pérez García',
  nombre_madrina:'Patricia Rangel García',
  frase:'Hay momentos en la vida que son especiales por si solos...Compartirlos con las personas que quieres los convierte en inolvidables.',
  img_header:'',
  img_banner1:'',
  img_banner2:'',
  fecha_dia:'30',
  fecha_mes:'Noviembre',
  fecha_year:'2024',
  religiosa_hora:'17',
  religiosa_minutos:'00',
  religiosa_lugar:'Iglesia de la Inmaculada Concepcion de María',
  religiosa_direccion:'Calzada del Ferrocarril',
  religiosa_direccion_col:'Centro',
  religiosa_direccion_cd:'Iguala de la Independencia',
  religiosa_ubicacion:'https://maps.app.goo.gl/wbCywQgfxSX3H68y8',
  recepcion_hora:'18',
  recepcion_minutos:'30',
  recepcion_lugar:'Jardín San Andrés',
  recepcion_direccion:'calle el Mezquital, 7',
  recepcion_direccion_col:'',
  recepcion_direccion_cd:'Iguala de la Independencia',
  recepcion_ubicacion:'https://maps.app.goo.gl/xbCuATV3YphsZF4F8',
  confirmacion:'7445299131',
  regalos_frase:'Lo más importante para mí es tu presencia a mis XV años, si deseas tener un presente conmigo puede ser en efectivo',
  regalos_url:'',
  time_line:{ 
    settings:{
      bgColor:'#f1ede6',
      color:'',
      lineColor:'#f1ede6'
    },
    protocol:[
      {
        id:1,
        time:'17:00 Hrs.',
        name:'Ceremonia Religiosa',
        icon:'iglesia',
        color:'',
        textColor:''
      },

      {
        id:2,
        time:'18:30 Hrs.',
        name:'Recepcion',
        icon:'recepcion',
        color:'',
        textColor:''
      },

      {
        id:3,
        time:'21:00 Hrs.',
        name:'Cena',
        icon:'cutlery_white',
        color:'',
        textColor:''
      },
      {
        id:4,
        time:'22:00 Hrs. en adelante',
        name:'Todos a bailar',
        icon:'music_white',
        color:'',
        textColor:''
      },
    ]
  },
  hoteles:[
    {
      nombre:'Rinconada Hotel Boutique',
      direccion:'Gral. Baltazar R. Leyva Manzilla, 19, Centro',
      direccion_colonia:'Iguala de la Independencia, Gro',
      telefono:'7333332158',
      ubicacion:'https://maps.app.goo.gl/GK7ARLS9ESHJT2pa8'
    },
    {
      nombre:'Hotel Ricarlo',
      direccion:'Periférico Oriente, 13',
      direccion_colonia:'Iguala de la Independencia, Gro',
      telefono:'7331102516',
      ubicacion:'https://maps.app.goo.gl/UxgemAM2e6eTe4Q29'
    },
    {
      nombre:'Hotel Real 1900',
      direccion:'Miguel Hidalgo, 6, Centro',
      direccion_colonia:'Iguala de la Independencia, Gro.',
      telefono:'7331101106',
      ubicacion:'https://maps.app.goo.gl/bcRQiHRzWotJcCNe9'
    },
    {
      nombre:'Gran Plaza Hotel Centro Historico',
      direccion:'Miguel Hidalgo, 7, Centro',
      direccion_colonia:'Iguala de la Independencia, Gro.',
      ubicacion:'https://maps.app.goo.gl/KpSXNAmm8gdbRCeB6'
    }
  ]
}